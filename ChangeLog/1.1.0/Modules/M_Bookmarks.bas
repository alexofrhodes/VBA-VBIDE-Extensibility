...
Sub SaveBookmark(index As Long)
    ...
    Dim lineIndex   As Long: lineIndex = aCodeModule.Initialize(Module).firstRow
...
Sub RemoveBookmark(index As Long)
    ...
    ap.Initialize wb, Module, Procedure
    For i = ap.lines.last To ap.lines.first Step -1
...
Retry:
    ...
    If Not ProcedureExists(wb, Procedure) Then
        ...
        Exit Sub
    End If
    Dim ap          As New aProcedure: ap.Initialize wb, Module, Procedure
    ProcFirstline = ap.lines.first
    Module.CodeModule.CodePane.SetSelection ProcFirstline, 1, ProcFirstline, 1
    If BookmarkLine <> "" Then
        If ap.code.Contains(BookmarkLine, False, False, False) Then
            Dim i   As Long
            For i = ProcFirstline To ap.lines.last
