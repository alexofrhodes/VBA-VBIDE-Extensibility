...
'--------------------------------------------------
' Fun PushVersionInitial  As Variant  :
' Fun PushVersionMajor    As Variant  :
' Fun PushVersionMinor    As Variant  :
' Fun PushVersionPatch    As Variant  :
' Fun dateAfterLastPush   As Variant  :
'--------------------------------------------------
...
Normal_Exit:
    ...
    TxtOverwrite TargetWorkbook.path & "\" & aProject.Initialize(TargetWorkbook).NameClean & "_ChangeLog.txt", PrettyPrint.ArrayToTable(Table.Range.value, True)
    Dim targetFolder As String
    ...
    With aProject.Initialize(TargetWorkbook)
        If chWorkbookBackup Then TargetWorkbook.SaveCopyAs targetFolder & .Name
        If chExportReferences Then .REFERENCES.Export targetFolder
...
'@INCLUDE PROCEDURE ArrayTrim
    ...
    arr = ArrayTrim( _
            Split( _
                aProject.Initialize(TargetWorkbook).code, _
                vbNewLine))
    arr = ArrayTrim( _
            Filter( _
                Filter( _
...
Private Sub LB_Books_Change()
    ...
    Dim obj
    On Error Resume Next
    Set obj = TargetWorkbook.VBProject.VBComponents
    On Error GoTo 0
    
    If Not obj Is Nothing Then ' aProject.initialize(TargetWorkbook).HasProject Then
