...
Private Sub exportDeclarationsAndCalls_Click()
    ...
    If collections(1).count > 0 Then
        ...
        End With
    End If
    Set collections = aProject.Initialize(skellyBook).Declarations.Items(True, True, True, True, True, True)
    If collections(1).count > 0 Then
        var = aCollection.CollectionsToArray2D(collections)
...
Sub loadComponents()
    ...
    For Each skellyModule In skellyBook.VBProject.VBComponents
        LComponents.AddItem
        LComponents.List(LComponents.ListCount - 1, 0) = aModule.Initialize(skellyModule).TypeToString
...
Private Sub LComponents_Click()
    ...
    Next
    TComps.text = aModule.Initialize(skellyModule).code
...
Private Sub LProcedures_Click()
    ...
    'test
    Dim coll        As Collection: Set coll = aProject.Initialize(skellyBook).Declarations.Items(True, True, True, True, True, True)
